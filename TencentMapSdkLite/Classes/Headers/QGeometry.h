...
/**地图状态*/
typedef struct {
    CGFloat     scale;       // 缩放比例, 对应于QMapView的mapScale属性,3－19级, 不同于zoomLevel
    CGFloat     rotation;    // 旋转角度
    CGFloat     overlooking; // 俯视角度
    
    CGPoint     screenPoint;//屏幕坐标（地理坐标对应的屏幕坐标）
    CLLocationCoordinate2D centerCoordinate;//地理坐标（中心点）
} QMapStatus;
